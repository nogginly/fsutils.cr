    ...
    module Definitions
      ...
      def fetch_as_md(config : Config = Config.new) : Definition
        Definition.new(
          ...
          description: "Fetch a URL and return it as Markdown. An HTML page is converted to Markdown; a site that serves Markdown is used as it is; any other text -- CSV, JSON, XML, CSS, SVG, plain text -- is returned inside a fenced code block tagged with its type, so the content reaches you unchanged. `content_type` says which of those happened. Short results are returned in `content`. A longer one is written to a file in the workspace and described instead: `path` is where it went, `excerpt` is its opening, and `toc` lists its headings with the line each section starts and ends at -- pass those to #{Names::READ} as `offset` and `limit` to read one section without reading the whole file. A stored file opens with a front matter block naming the page it came from, because its links are relative to that address rather than to the workspace. Check `truncated` and `notice`: content over #{config.fetch.max_content_bytes} bytes is cut short. Only text is read, and only public http and https addresses may be fetched.",
          schema: <<-JSON
            {
              "type": "object",
              "properties": {
